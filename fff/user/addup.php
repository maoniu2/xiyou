...
if(isset($get['lv'])){
	...
	$sql=" `lv`='$lv' and `type`='1'";
}else{
	$sql=" `type`='1'";
}
//图标
$fotter29=$DB->getRow("select * from `tubiao` where `id`='29' limit 1");
?>
<div class="kbox"></div>
<div class="kbox"></div>
<div class="clear"></div>
...
<div class="con">
    <ul>
      <li>
        <a href="./addup.php">
          <div class="ddimg">
			<img src="<?php echo $fotter29['value']; ?>" />
          </div>
          <p>全部奖励</p>
        </a>
      </li>
		...
		while($res = $rs->fetch())
		{
			$allljslnum=$DB->getColumn("SELECT count(*) from `addup` WHERE `lv` = '".$res['id']."' and `type`='1'");
			if($allljslnum > 0){
			$value = explode('.',$res['value']);
			echo '
			<li>
				<a href="./addup.php?lv='.$res['id'].'">
				<div class="ddimg">
					<img src="'.$res['image'].'" />
				</div>
				<p>累计'.$value[0].'</p>
...
<div class="con">
	...
	while($res = $rs->fetch())
	{
		...
		if($bindData['charge'] >= $lvData['value']){
			if(strpos($bindData['chargelq'],'['.$res['id'].']') !== false){ 
			...
			$lingqu = '<div class="dpbtn2"><a style="color: #00FFFF">未达到要求</a></div>';
		}
		echo '
		<div class="message_1">
			<div class="meL">
			...
			<div class="meR_2">领取要求：角色累计满'.$lvData['value'].'元</div>
			</div>
		</div>
	  ';
	}
